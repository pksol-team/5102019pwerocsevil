...
class GF_SendGrid extends GFAddOn {
	...
	public function maybe_send_email( $email, $message_format, $notification, $entry ) {
		...
		if ( $email['abort_email'] ) {
			$this->log_debug( __METHOD__ . '(): Not sending email via SendGrid because the notification has already been aborted by another Add-On.' );
			return $email;
		}
		...
		$sendgrid_email = array(
			'from' => array(
				'email' => $from_email[1],
				'name' => rgar( $notification, 'fromName' ) ? rgar( $notification, 'fromName' ) : null
			),
		...
		if ( rgar( $notification, 'bcc' ) ) {
			$sendgrid_email['personalizations'][0]['bcc'][]['email'] = GFCommon::replace_variables( rgar( $notification, 'bcc' ), $form, $entry, false, false, false, 'text' );
		}
		...
		try {
			...
			$this->log_debug( __METHOD__ . '(): Sending email via SendGrid; ' . print_r( $sendgrid_email, true ) );

			// Send email.
			$sent_email = $this->api->send_email( $sendgrid_email );

			// Log that email was sent.
			$this->log_debug( __METHOD__ . '(): Email successfully sent by SendGrid; ' . print_r( $sent_email, true ) );
			...
			$this->log_error( __METHOD__ . '(): Unable to send email with SendGrid; ' . $e->getMessage() );
