...
class GFCampfire extends GFFeedAddOn {
	...
	public function plugin_settings_description() {
		...
		$description  = '<p>';
		$description .= sprintf(
			esc_html__( 'Campfire provides a simple group chat for your team. Use Gravity Forms to alert your Campfire rooms of a new form submission. If you don\'t have a Campfire account, you can %1$ssign up for one here.%2$s', 'gravityformscampfire' ),
			'<a href="http://www.campfirenow.com/" target="_blank">', '</a>'
		);
		$description .= '</p>';
		
		if ( ! $this->initialize_api() ) {
			$this->log_error( 'API could not be initialized.' );
			$description .= '<p>';
	...
	public function initialize_api() {
		...
		// If this is called when new settings are being saved, they are not yet saved and the posted data needs to be used.
		...
		try {
			...
			/* Run API test. */
			$rooms = $campfire->get_rooms();
			// If access was denied, the string "HTTP Basic: Access denied." is returned. This should be considered invalid credentials.
			if ( $rooms == 'HTTP Basic: Access denied.' ){
				$this->log_error( __METHOD__ . '(): API credentials are invalid; Message: ' . $rooms  );
			...
			$this->log_error( __METHOD__ . '(): API credentials are invalid; Message: '. $e->getMessage() . ' Code: ' . $e->getCode() );
	...
	public function validate_account_url( $account_url ) {
		...
		try {
			...
			$this->log_debug( __METHOD__ . "(): testing URL" );
			$campfire->validate_account_url();
			$this->log_debug( __METHOD__ . "(): URL tested" );
