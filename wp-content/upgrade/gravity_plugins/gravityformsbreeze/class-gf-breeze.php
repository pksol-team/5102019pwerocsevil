...
class GF_Breeze extends GFFeedAddOn {
		...
		$base_fields = array(
			...
			'fields' => array(
				...
				array(
					...
					'type'           => 'select',
					'required'       => true,
					'choices'        => $this->get_projects_for_feed_setting(),
	...
	public function get_lists_for_feed_setting() {
		...
		$settings = rgar( $settings, 'meta' ) ? $settings['meta'] : $settings;

		// Get the lists for Breeze project.
		$lists = $this->api->get_project_lists( $settings['project'] );
	...
	public function get_users_for_feed_setting() {
		...
		$project = $this->api->get_project( $settings['project'] );
	...
	public function get_column_value_project( $feed ) {
		...
		try {
			...
			return isset( $project['name'] ) ? $project['name'] : $project_id;

		} catch ( Exception $e ) {

			// Log the request error.
			$this->log_error( __METHOD__ . '(): Unable to get Breeze project; ' . $e->getMessage() );
	...
	public function process_feed( $feed, $entry, $form ) {
		...
		try {
			...
			// Add feed error.
			$this->add_feed_error( esc_html__( 'Card could not be created: ', 'gravityformsbreeze' ) . $e->getMessage(), $feed, $entry, $form );
