...
class GF_Partial_Entries extends GFFeedAddOn {
	...
	public function filter_gform_filter_links_entry_list( $filter_links, $form, $include_counts ) {
		...
		if ( $include_counts ) {
			...
			$sql            = $wpdb->prepare(
				"SELECT
                    (SELECT count(DISTINCT(l.id)) FROM $lead_table_name l INNER JOIN $lead_detail_meta_name m ON l.id=m.{$entry_id_column} WHERE l.form_id=%d AND l.status='active' AND m.meta_key = 'partial_entry_percent' AND m.meta_value = '' ) as complete,
